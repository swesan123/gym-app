-- Soft delete for workouts so a deletion (accidental or deliberate) is
-- recoverable for a window instead of losing history permanently with no
-- audit trail — prompted by a user-reported missing Week 28 Push workout
-- that could not be traced or restored.
ALTER TABLE workouts
ADD COLUMN IF NOT EXISTS deleted_at TIMESTAMP WITH TIME ZONE DEFAULT NULL;

CREATE INDEX IF NOT EXISTS idx_workouts_deleted_at ON workouts (deleted_at);

-- Rep-capacity progress views must exclude soft-deleted workouts; definitions
-- are otherwise unchanged from 20260712000000_rep_capacity_views_replace_volume.sql.
